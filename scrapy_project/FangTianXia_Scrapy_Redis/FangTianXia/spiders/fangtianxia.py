...
"""
    普通爬虫改为分布式爬虫：
    1.需要导入scrapy-Redis，爬虫的类继承需要改为基础Redis的spider：from scrapy_redis.spiders import RedisSpider(注意：如果爬虫是crawlspider，需要改为对应的类)
    2.删除或者注释掉：start_urls，采用redis-key代替,redis-key可以自定义
    3.是通过定义的Redis的key来获取url，不是从start_urls里面取，开始的url会插入到我们定义的Redis的key里面的列表：redis_key = "fang:start_urls"
    4.修改调度器的配置，采用Redis的
...
class FangtianxiaSpider(RedisSpider):
    ...
    redis_key = "fang_tian_xia:start_urls"   #是通过定义的Redis的key来获取url，不是从start_urls里面取
